...
class JavelParser {
    ...
    *iter(ja) { // 全文パース（ジェネレータ）
        // Loading表示
        for (let block of TextBlock.iter(ja)) {
            this.htmlBlocks.push(this.blockToHtml(block))
            yield this.htmlBlocks
        }
        // Loading非表示
    }
