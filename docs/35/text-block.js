(function(){
class TextBlock {
    static reactive(text) { return vanX.reactive(this.fromText(text)) }
    ...
    static *iter(text) {
        const [blocks, block] = [[], []]
        for (let line of this.#textToLines(text)) {
            block.push(line)
            //if (''===line && 0 < block.length) { blocks.push(block.join('\n').trimLine()); block.splice(0); }
            if (''===line && 0 < block.length) { yield block.join('\n').trimLine(); block.splice(0); }
        }
        //if (0 < block.length) { blocks.push(block.join('\n').trimLine()) }
        if (0 < block.length) { yield block.join('\n').trimLine(); block.splice(0); }
//        return blocks.filter(v=>v)
    }
...
//        if (cutText.endsWith('\n\n')) { return (cutText.startsWith('\n') && textFront.endsWith('\n')) ? 2 : 1 }
    ...
    static pasteBlocks(selectionStart, selectionEnd, pasteText, text, blocks) {
        ...
        console.debug('blockBack.trimLine()==nextBlockText:', blockBack.trimLine()==nextBlockText)
        console.debug('(blockFront + pasteText + blockBack):', (blockFront + pasteText + blockBack))
        console.debug('newBlockText:', (blockFront + pasteText + blockBack) + (((blockBack.trimLine()==nextBlockText) || (cutText.startsWith('\n') && cutText.endsWith('\n') && textFront.endsWith('\n') && textBack.startsWith('\n'))) ? '' : '\n\n' + nextBlockText))
        console.debug('newBlockTextLines:', ((blockFront + pasteText + blockBack) + (((blockBack.trimLine()==nextBlockText) || (cutText.startsWith('\n') && cutText.endsWith('\n') && textFront.endsWith('\n') && textBack.startsWith('\n'))) ? '' : '\n\n' + nextBlockText)).split(/\r?\n/))
