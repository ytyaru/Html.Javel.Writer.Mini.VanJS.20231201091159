...
class SceneBuilder {
    ...
    setChildren(sid, eid, children) {
        if (Type.isEl(children) || Type.isFn(children)) { children = [children] }
        if (!Type.isEls(children) && !Type.isFns(children)) { throw new Error(`setChildrenの第三引数はHTML要素またはそれを返す関数の配列であるべきです。:${typeof children}, ${children}`) }
