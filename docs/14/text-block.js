...
//        console.log((blockBack.trimLine()==nextBlockText))
        ...
        console.log(`cutText: ${(cutText.match(/^[\n]{2,}/g) || []).length} : ${(cutText.match(/[\n]{2,}$/g) || []).length} : ${cutText}`)
        console.log(`cutText: ${cutText.startsWith('\n')} : ${cutText.endsWith('\n')}`)
        const isCutBlock = (cutText.startsWith('\n') && cutText.endsWith('\n')) && (textFront.endsWith('\n') && textBack.startsWith('\n'))
        console.log(`isCutBlock: ${isCutBlock}`)
        //const isCutBlock = (cutText.startsWith('\n') && cutText.endsWith('\n')) && ()
        ...
        //const nextBlockText = (index+1<blocks.length) ? blocks[index+1+((cutText.startsWith('\n') && cutText.endsWith('\n')) ? 1 : 0)] : ''
...
//        const insertText = (blockFront+blockBack+((cutText.endsWith('\n') ? '' : '\n\n'+nextBlockText))).trimLine()
        ...
        const insertText = (blockFront+((isCutBlock) ? '' : blockBack)+(((cutText.endsWith('\n') && textFront.endsWith('\n') && textBack.startsWith('\n')) ? '' : '\n\n'+nextBlockText))).trimLine()
        //const insertText = (blockFront+((isCutBlock) ? '' : blockBack)+((Array.from(cutText).every(c=>'\n'===c)) ? '' : '\n\n'+nextBlockText)).trimLine()
