...
class JavelParser {
    ...
    toHtmls(ja) { // 全文パース
        ...
        //this.textBlocks = this.#linesToBlocks(this.#textToLines(ja))
    ...
    setBlockText(blockIndex, ja) {
        ...
        //this.htmlBlocks[blockIndex] = this.#blocksToHtmls([this.textBlocks[blockIndex]]).flat()
        //this.htmlBlocks[blockIndex] = this.#blocksToHtmls([this.textBlocks[blockIndex]])[0]
        this.htmlBlocks[blockIndex] = this.#blocksToHtmls([ja])[0]
        return this.htmlBlocks
    }
    blockToHtml(text) { return this.#blocksToHtmls([text])[0] }
    cutBlocks(index, cutCount) {
        this.textBlocks.splice(index, cutCount)
        this.htmlBlocks.splice(index, cutCount)
        return this.htmlBlocks
    }
...
//        this.htmlBlocks.splice(index, deleteCount, ...this.#blocksToHtmls(blocks))
    ...
    insertBlocks(index, blocks, deleteCount=0) {
        this.textBlocks.splice(index, deleteCount, ...blocks)
        this.htmlBlocks.splice(index, deleteCount, ...this.#blocksToHtmls(blocks))
        return this.htmlBlocks
    }

    changeBlocks(blockIndex, ja) {
    ...
    paste(ja, blockIndex) { // ペーストした箇所だけパースする（負荷軽減）
        ...
        const blocks = TextBlock.fromText(ja)
...
//        console.debug('Em:', Em.matches(text).map(m=>m.html.outerHTML))
//        console.debug('Ruby.Long:', Ruby.matchesLong(text).map(m=>m.html.outerHTML))
//        console.debug('Ruby.Short:', Ruby.matchesShort(text).map(m=>m.html.outerHTML))
